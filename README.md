...
The [Docker](Docker) directory holds a [Dockerfile](https://docs.docker.com/engine/reference/builder/) and a dedicated configuration file (`theonionbox.cfg`) to support the operation of *The Onion Box* Docker images are avialable on [hub.docker.com](https://hub.docker.com/search?q=theonionbox&type=image). Please be aware, that these Docker images for *The Onion Box* only supports [password authentication](#password-authentication).
